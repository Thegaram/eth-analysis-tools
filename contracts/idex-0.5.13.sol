...
contract Exchange {
  function assert2(bool assertion) private pure {
    if (!assertion) revert();
  }
  ...
  function adminWithdraw(address token, uint256 amount, address payable user, uint256 nonce, uint8 v, bytes32 r, bytes32 s, uint256 feeWithdrawal) public onlyAdmin returns (bool) {
    bytes32 hash = keccak256(abi.encodePacked(address(this), token, amount, user, nonce));
  ...
  function trade(uint256[8] memory tradeValues, address[4] memory tradeAddresses, uint8[2] memory v, bytes32[4] memory rs) public onlyAdmin returns (bool) {
    ...
    bytes32 orderHash = keccak256(abi.encodePacked(address(this), tradeAddresses[0], tradeValues[0], tradeAddresses[1], tradeValues[1], tradeValues[2], tradeValues[3], tradeAddresses[2]));
